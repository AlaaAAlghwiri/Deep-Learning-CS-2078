...
def get_batch(D, context_size, batch_size):
    ...
    """
    n = len(D)
    starts = torch.randint(0, n - context_size - 1, (batch_size,))  ### randmization for batching 
    x = torch.stack([D[s:s+context_size] for s in starts]).T  # the shape will be  (context_size, batch_size)
    y = torch.stack([D[s+1:s+1+context_size] for s in starts]).T # TODO fill in the code to get the target data
...
def train(Dtrain, Dval, model, optimizer, criterion, context_size, batch_size, vocab_size, num_epochs, save_freq=10, save_name="transformer"):
    """
    ...
    for epoch in tqdm(range(num_epochs)):  # print out progress bar
        ...
        for _ in (range(updates_per_epoch)):  # loop over the dataset
            ...
            loss = criterion(y_pred.reshape(-1, vocab_size), y.reshape(-1))
...
def get_data():
    ...
    with open(fname, 'r', encoding='utf-8') as f:
        text = f.read()
    # TODO create a list of characters. Probably best to sort them if you want to have a consistent mapping
...
def train_main(dtrain, dval, vocab_size, params):
    ...
    model, tlosses, vlosses = train(dtrain, dval, model, optimizer, criterion,
                                    params['context_size'], params['batch_size'], vocab_size, params['num_epochs'], 
...
if __name__ == "__main__":
    ...
    cs=256  # context size
    nl=8 # number of transformer layers
