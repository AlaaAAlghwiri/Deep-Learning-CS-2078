...
def relu(x):
    ...
    """
    out = np.maximum(0, x)  ## TODO replace with code to compute the ReLU function
    back = lambda dout: dout * (x >= 0)  ## TODO replace with code to compute the derivative of the ReLU function times dout
    return out, back
...
def linear_forward(x, W, b):
    ...
    """
    z = x.dot(W.T) + b  ## TODO replace with code to compute the output of the linear layer
...
def linear_backward(dz, x, W):
    ...
    """
    dx = dz.dot(W)  ## TODO replace with code to compute the partial derivative of the loss with respect to x
    dw = dz.T.dot(x)  ## TODO replace with code to compute the partial derivative of the loss with respect to W
    db = np.sum(dz, axis=0)  ## TODO replace with code to compute the partial derivative of the loss with respect to b
    return dx, dw, db
...
class DenseLayer(object):
    def __init__(self, num_inputs, num_outputs, activation=None):
        ...
        """
        self.W = np.random.randn(num_outputs, num_inputs) * (1.0 / np.sqrt(num_inputs)) ## TODO replace with code to initialize the weights
...
class NN(object):
    ...
    def forward(self, x):
        ...
        """
        # TODO read and understand the code below
        backs = []
...
def cross_entropy_loss(h, y):
    ...
    p = exp_h / sum_exp_h 
    loss = -np.sum(y * np.log(p)) / m  ## TODO replace with code to compute the loss
    dh = (p - y) / m  ## TODO replace with code to compute the gradient of the loss with respect to h
