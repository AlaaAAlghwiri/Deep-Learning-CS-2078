...
class BatchGenerator(object):
    ...
    def __next__(self):
        ...
        if self.current_batch >= self.num_batches:
            raise StopIteration
        #return self.x, self.y ## TODO replace with code to return the next batch (use self.current_batch to compute start and end points)
    ...
    def shuffle(self):
        ...
        self.y = self.y[indices]  ## TODO replace with code to randomly reorder the data
...
def train(model, data, optimizer, num_epochs=100, call_back=None):
    ...
    for epoch in range(num_epochs):
        ...
        for x_batch, y_batch in data:
            h, back = model.forward(x_batch)  ## TODO replace with code to get output and backprop function for the network
            loss, dl = cross_entropy_loss(h, y_batch) ## TODO replace with code to compute the loss and its derivative with respect to h
...
def make_one_hot(y, num_classes):
    ...
    """
    # TODO see how the class label is represented. 
